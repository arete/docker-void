...
RUN tar -Jxvf /root/xbps-static-latest.x86_64-musl.tar.xz -C / && mkdir -p /root/voidlinux/root  /root/voidlinux/cache &&  yes|/usr/bin/xbps-install -Syu -R https://repo.voidlinux.eu/current  xbps-triggers coreutils base-files glibc-locales dash grep xbps iana-etc tzdata sed  -r shadow /root/voidlinux/root -c /root/voidlinux/cache
